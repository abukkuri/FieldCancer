...
def run_this():
    ...
    while t[-1] < tend:
        ...
        if tx_on==False and pre[-1]+mal[-1] > normal[-1]:
            tx_on = True

        rates = [current_n * rN * (K1 - current_pre - current_n) / K1, current_n * dN,
                 current_pre * rP * (K1 - current_pre - current_n) / K1, current_pre * (dN + cP()),
                 current_mal * rM* (K2 - current_mal) / K2, current_mal * (dN + cM())]
...
fig, ax = plt.subplots()
ax.set_title('Restoration of Signaling Integrity')
